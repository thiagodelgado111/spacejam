...
(function() {
  var ChildProcess, Meteor, chai, expect, path, ps, sinon, sinonChai, _;
  ...
  describe("Meteor", function() {
    ...
    describe("getTestArgs()", function() {
      ...
      it("get common args for test and test-packages command", function() {
        var args, options;
        options = {
          "driver-package": "package",
          "release": 'release',
        ...
        return expect(args).to.be.deep.equal(["test", "--driver-package", "package", "--release", "release", "--port", "3000", "--settings", "settings", "--production"]);
      });
      it("create args for test-packages command", function() {
        var args;
        args = meteor.getTestArgs('test-packages', this.options);
      ...
      it("create args for test command", function() {
        var args;
        _.extend(this.options, {
          "test-app-path": "/tmp/app",
          "full-app": true
        });
        args = meteor.getTestArgs('test', this.options);
      ...
      return it("use package practicalmeteor:mocha if mocha practicalmeteor:mocha-console-runner as driver-package", function() {
        var args, expectedArgs, opts;
        expectedArgs = ["test", "--driver-package", "practicalmeteor:mocha", "--release", "release", "--port", "3000", "--settings", "settings", "--production", "--test-app-path", "/tmp/app", "--full-app"];
        opts = _.extend(_.clone(this.options), {
          "test-app-path": "/tmp/app",
          "full-app": true,
          "mocha": true
        });
        ...
        args = meteor.getTestArgs('test', opts);
        return expect(args, "--driver-package=practicalmeteor:mocha-console-runner").to.be.deep.equal(expectedArgs);
      });
    });
    describe("runTestCommand", function() {
      beforeEach(function() {
        this.expectedSpawnArgs = ["test", "--driver-package", "practicalmeteor:mocha", "--port", defaultTestPort];
        this.expectedSpawnOptions = getExpectedSpawnOptions(4096);
        return this.expectedSpawnOptions.env.MOCHA_REPORTER = 'console';
      });
      it("should spawn meteor with env var MOCHA_REPORTER to console if mocha option or practicalmeteor:mocha as driver-package", function() {
        meteor.runTestCommand("test", {
          "mocha": true
        });
        expect(spawnStub.args[0]).to.eql(["meteor", this.expectedSpawnArgs, this.expectedSpawnOptions]);
        meteor = new Meteor();
        meteor.runTestCommand("test", {
          "driver-package": "practicalmeteor:mocha"
        });
        return expect(spawnStub.args[0]).to.eql(["meteor", this.expectedSpawnArgs, this.expectedSpawnOptions]);
      });
      return it("should spawn meteor with env var MOCHA_REPORTER to console with practicalmeteor:mocha-console-runner as driver-package", function() {
        meteor.runTestCommand("test", {
          "driver-package": "practicalmeteor:mocha-console-runner"
        });
        return expect(spawnStub.args[0]).to.eql(["meteor", this.expectedSpawnArgs, this.expectedSpawnOptions]);
